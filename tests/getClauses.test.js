...
test('Creating WHERE clause', () => {  
    ...
    expect(dynamicWhere.getClauses()).toBe(' WHERE name = (?)');
...
test('Creating WHERE clause with leading operator', () => {  
    ...
    expect(dynamicWhere.getClauses(true)).toBe(' AND color = (?)');
...
test('Creating WHERE clause with multiple values', () => {  
    ...
    expect(dynamicWhere.getClauses()).toBe(' WHERE name = (?) AND age >= (?) OR height = (?)');
...
test('Creating WHERE clause with multiple values, some undefined', () => {  
    ...
    expect(dynamicWhere.getClauses()).toBe(' WHERE name = (?) OR height = (?)');
...
test('Creating WHERE clause with multiple values, some overrides', () => {  
    ...
    expect(dynamicWhere.getClauses()).toBe(' WHERE make = (?) OR year = (?)');
